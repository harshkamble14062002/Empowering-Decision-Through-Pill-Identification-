...
def load_local_env(path=Path(__file__).resolve().parent / ".env"):
...
class OpenRouterGenerator:
    ...
    def answer(self, medicine_name, question, evidence):
        ...
        facts = "\n".join(f"- {key}: {value}" for key, value in evidence.items() if value)
        system_prompt = (
        ...
        if getattr(response, "status_code", None) == 400:
            retry_payload = {key: value for key, value in payload.items()
        ...
        try:
            ...
            answer = content
        valid_script = bool(KANNADA_RE.search(answer)) if language == "Kannada" else bool(re.search(r"[A-Za-z]", answer)) and not KANNADA_RE.search(answer)
