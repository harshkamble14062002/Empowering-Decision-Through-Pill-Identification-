#!/usr/bin/env python3
"""Classify full-image OCR regions with CSV evidence, without YOLO."""
import argparse
import csv
import json
import re
...
from extract_text import _read_ocr

ROOT = Path(__file__).resolve().parent
DEFAULT_DATABASE = ROOT / "Medicine_Details_Verified.csv"
IGNORE_PHRASES = {
    "mrp", "batch", "batch no", "expiry", "expiry date", "mfg date",
    "manufacturing date", "use before", "schedule h", "keep out of reach",
    "store below", "for external use", "not for sale", "sample",
}
GENERIC_TOKENS = {
    "tablet", "tablets", "capsule", "capsules", "release", "sustained",
    "extended", "film", "coated", "strip", "strips", "ip", "usp", "bp",
    "contains", "each", "dosage", "colour", "warning", "rx", "only",
}
...
def brand_root(medicine_name):
    ...
    for token in tokens(medicine_name):
        if (token not in GENERIC_TOKENS | FORMULATION | DOSAGE_UNITS
...
def brand_evidence(line, medicine_name):
    ...
    words = [token for token in tokens(line["text"])
             if token not in GENERIC_TOKENS | DOSAGE_UNITS
             and re.search(r"[a-z]", token)]
    identity = max((fuzz.ratio(root, word) for word in words), default=0.0)
    if not root or identity < 65:
        return 0.0
    return .70 * identity + .30 * similarity(line["text"], medicine_name)
...
def same_line_groups(raw):
    ...
    for item in items:
        ...
        line = next((line for line in lines if abs(line["cy"] - item["cy"]) <= .55 * max(height, line["height"])), None)
        if line is None:
    ...
    for line in lines:
        ...
        output.append({
            ...
            "bbox": [float(min(p["x1"] for p in parts)), float(min(p["y1"] for p in parts)),
                     float(max(p["x2"] for p in parts)), float(max(p["y2"] for p in parts))],
        })
    return output
...
def preclassify_lines(lines, rows):
    ...
    values = {
        ...
        "composition": list({row.get("Composition", "") for row in rows if row.get("Composition")}),
        "manufacturer": list({row.get("Manufacturer", "") for row in rows if row.get("Manufacturer")}),
    }
    ...
    for line in lines:
        ...
        scores = {
            name: max((similarity(line["text"], value) for value in candidates), default=0)
            for name, candidates in values.items()
        }
        manufacturer_identity = bool(text_tokens) and any(
        ...
        if is_irrelevant(line["text"]):
        ...
        elif scores["composition"] >= 65 and scores["composition"] >= scores["brand_name"] + 5:
            ...
            class_name = "ignore"
        output.append({**line, "preliminary_class": class_name,
                       "global_scores": {key: round(value, 2) for key, value in scores.items()}})
    return output
...
def rank_rows(lines, rows):
    ...
    brand_lines = [line for line in lines
                   if line["preliminary_class"] not in {"composition", "manufacturer"}
                   and not is_irrelevant(line["text"])]
    ranked = []
    for row in rows:
        brand_options = [(brand_evidence(line, row["Medicine Name"]), line)
                         for line in brand_lines]
        comp_options = [(similarity(line["text"], row.get("Composition", "")), line) for line in by_class["composition"]]
        mfg_options = [(similarity(line["text"], row.get("Manufacturer", "")), line) for line in by_class["manufacturer"]]
        brand_score, brand_line = max(brand_options, default=(0, {"text": ""}), key=lambda item: item[0])
        ...
        if observed_forms and not observed_forms.issubset(expected_forms):
            penalty += 10
        total = .60 * brand_score + .30 * comp_score + .10 * mfg_score - penalty
        ranked.append({"row": row, "score": total, "brand": brand_score,
                       "composition": comp_score, "manufacturer": mfg_score,
...
def classify_lines(lines, row):
    ...
    for line in lines:
        scores = {name: similarity(line["text"], expected) for name, expected in fields.items()}
        name = line["preliminary_class"]
        if name == "ignore" or scores[name] < thresholds[name]:
            name = "ignore"
        output.append({key: value for key, value in line.items() if key != "preliminary_class"} | {
            "normalized_text": normalize(line["text"]), "class_name": name,
...
def analyze(image_path, database_path=DEFAULT_DATABASE, output_path=None):
    ...
    raw = [item for item in raw if float(item[2]) >= .25]
    ...
    acceptance_policy = ("combined_evidence" if combined_evidence
                         else "strong_unique_brand" if strong_unique_brand
                         else None)
    classifications = classify_lines(lines, best["row"]) if accepted else [
        {key: value for key, value in line.items() if key != "preliminary_class"} | {"normalized_text": normalize(line["text"]), "class_name": "ignore", "scores": {}}
        for line in lines
    ]
    result = {
        "status": "success" if accepted else "uncertain",
        "medicine": best["row"] if accepted else None,
        "score": round(best["score"], 2), "margin": round(margin, 2),
        "field_scores": {key: round(best[key], 2) for key in ("brand", "composition", "manufacturer")},
        "acceptance_policy": acceptance_policy,
        "ocr_regions": classifications,
        "candidates": [{"medicine_name": item["row"]["Medicine Name"], "score": round(item["score"], 2)} for item in ranked[:3]],
    }
    if output_path:
        colors = {"brand_name": (0, 180, 0), "composition": (255, 150, 0),
                  "manufacturer": (0, 0, 220), "ignore": (150, 150, 150)}
        preview = image.copy()
        for item in classifications:
            x1, y1, x2, y2 = map(int, item["bbox"])
            color = colors[item["class_name"]]
            cv2.rectangle(preview, (x1, y1), (x2, y2), color, 2)
            cv2.putText(preview, item["class_name"], (x1, max(14, y1 - 4)),
...
def run_ocr_csv_pipeline(image_path, database_path=DEFAULT_DATABASE, output_dir="output", quiet=False):
    """Adapt the no-YOLO classifier to the application's identification contract."""
    started = time.time()
    ...
    return {
        ...
        "stages": {
            "detection": {"method": "full_image_easyocr", "annotated_path": str(annotated_path)},
            "ocr_csv": classified,
        },
    }
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "output/ocr_csv_classified.jpg")
    parser.add_argument("--save-json", type=Path)
    args = parser.parse_args()
    result = analyze(args.image, args.database, args.output)
