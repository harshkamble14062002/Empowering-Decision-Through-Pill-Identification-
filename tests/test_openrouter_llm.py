...
def test_grounded_request_and_model_metadata():
    ...
    with patch("openrouter_llm.requests.post", return_value=response) as post:
        generator = OpenRouterGenerator(api_key="test-secret")
        answer = generator.answer(
            "azithral 500 tablet", "ಇದರ ಉಪಯೋಗ ಏನು?",
    ...
    assert "Treatment of bacterial infections" in request["json"]["messages"][1]["content"]
...
def test_english_question_requires_english_answer():
    ...
    response.json.return_value = {
        "model": "provider/example:free",
        "choices": [{"message": {"content": '{"answer_text":"It is used for bacterial infections."}'}}],
    }
    with patch("openrouter_llm.requests.post", return_value=response) as post:
        answer = OpenRouterGenerator(api_key="test-secret").answer(
            "azithral 500 tablet", "What is this medicine used for?",
            {"uses": "Treatment of bacterial infections"},
        )
...
def test_free_model_400_retries_without_structured_output():
    ...
    accepted.json.return_value = {
        ...
        "choices": [{"message": {"content": "ಈ ಔಷಧಿಯನ್ನು ಅಲರ್ಜಿ ಚಿಕಿತ್ಸೆಗೆ ಬಳಸುತ್ತಾರೆ."}}],
    }
    complete_row = {
    ...
    with patch("openrouter_llm.requests.post", side_effect=[rejected, accepted]) as post:
        answer = OpenRouterGenerator(api_key="test-secret").answer(
