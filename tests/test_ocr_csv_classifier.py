...
from ocr_csv_classifier import brand_evidence, is_irrelevant, preclassify_lines, rank_rows
...
class OcrCsvClassifierTests(unittest.TestCase):
    def setUp(self):
        ...
        self.lines = [
            {"text": text, "confidence": .9, "bbox": [0.0, float(i), 10.0, float(i + 1)]}
    ...
    def test_brand_evidence_rejects_strength_only_and_composition_text(self):
        self.assertGreaterEqual(brand_evidence({"text": "Sonaxa 75"}, "sonaxa 75 capsule"), 99)
        self.assertEqual(brand_evidence({"text": "75"}, "renerve p 750mcg/75mg capsule"), 0)
        self.assertEqual(
            brand_evidence({"text": "Pregabalin"}, "genericart pregabalin 75mg capsule"), 0
        )
    ...
    def test_manufacturer_cannot_compete_as_brand(self):
        ...
        self.assertEqual(rank_rows(classified, self.rows)[0]["row"]["Medicine Name"],
