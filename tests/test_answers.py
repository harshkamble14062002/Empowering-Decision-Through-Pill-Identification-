...
from app.rag import answer_kannada, detect_intent, detect_language
...
def test_generator_receives_complete_matched_csv_row(tmp_path):
    ...
    index = sample_index(tmp_path)
    result = answer_kannada("azithral 500 tablet", "ಇದರ ಉಪಯೋಗ ಏನು?", index, Generator())
...
def test_english_question_gets_english_fallback(tmp_path):
    index = sample_index(tmp_path)
    result = answer_kannada(
        "azithral 500 tablet", "What is this medicine used for?", index
    )
    assert result["status"] == "success"
