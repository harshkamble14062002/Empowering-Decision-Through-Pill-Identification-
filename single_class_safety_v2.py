"""Conservative safety gate for the selected single-class pipeline."""
...
VARIANT_MARKERS = {
    "am", "bid", "cr", "ct", "dsr", "forte", "m", "mr", "mv", "nt",
    "plus", "sr", "trio", "xl",
}
...
def verify_single_candidate(ocr_text, ocr_confidence, candidate):
    if not candidate:
        return {"status": "no_match", "reasons": ["no_candidate"], "candidate": None}
    ...
    observed_markers = set(observed) & VARIANT_MARKERS
    ...
    if observed_markers - name_markers:
        reasons.append("candidate_variant_conflict")
    if not name_numbers and observed_numbers:
        reasons.append("strengthless_candidate_with_numeric_ocr")
